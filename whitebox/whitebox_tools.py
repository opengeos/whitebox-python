...
class WhiteboxTools(object):
    ...
    def multiscale_topographic_position_image(
        self, local, meso, broad, output, lightness=1.2, callback=None
    ):
    ...
    def change_in_contributing_area(
        self, dem, output, exponent=1.0, threshold=None, log=False, callback=None
    ):
        ...
        if log:
            args.append("--log")
        return self.run_tool(
            "change_in_contributing_area", args, callback
        )  # returns 1 if error
    ...
    def classify_overlap_points(
        self, i, output, resolution=2.0, filter=False, callback=None
    ):
    ...
    def vector_stream_network_analysis(
        self, streams, dem, output, cutting_height=10.0, snap=0.1, callback=None
    ):
